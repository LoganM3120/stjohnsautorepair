          ...
          <div className="stack">
            ...
            <div style={{ display: 'grid', gap: '1rem', marginTop: '1rem' }}>
              <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                <span style={{ width: '0.75rem', height: '0.75rem', borderRadius: '9999px', background: 'var(--accent)' }} />
                <p className="helper-text">Same-day appointments often available</p>
              </div>
              <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                <span style={{ width: '0.75rem', height: '0.75rem', borderRadius: '9999px', background: 'var(--secondary)' }} />
            ...
            <div style={{ display: 'grid', gap: '1rem' }}>
              {["Advanced diagnostics", "Preventative maintenance", "Electrical &amp; mechanical repairs", "Fleet service partnerships"].map((item) => (
                <div key={item} style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                  <span style={{ width: '2.25rem', height: '2.25rem', borderRadius: '0.75rem', background: 'rgba(248, 250, 252, 0.1)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <section className="section section--dark">
        <div className="container">
          <div className="stack" style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
            <span className="badge" style={{ background: 'rgba(250, 204, 21, 0.15)', color: '#facc15' }}>What we do</span>
          ...
          <div className="grid grid--two">
            {serviceHighlights.map((service) => (
              <article key={service.title} className="card" style={{ background: 'rgba(15, 23, 42, 0.85)', color: '#f8fafc' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.25rem' }}>
                  ...
                  <div>
                    ...
                    <p style={{ color: '#cbd5f5', fontSize: '0.95rem' }}>{service.subtitle}</p>
                  </div>
                </div>
                <ul style={{ display: 'grid', gap: '0.65rem', color: '#e2e8f0', paddingLeft: '1rem' }}>
            ...
            <div className="stack">
              {valueProps.map((value) => (
                <div key={value.title} style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
                  <span style={{ width: '2.25rem', height: '2.25rem', borderRadius: '0.75rem', background: 'rgba(37, 99, 235, 0.12)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: 'var(--secondary)' }}>
                    {value.icon}
                  </span>
                  <div className="stack" style={{ gap: '0.35rem' }}>
                    <h3 style={{ fontSize: '1.15rem' }}>{value.title}</h3>
                    <p className="helper-text" style={{ fontSize: '0.95rem', color: 'var(--muted)' }}>{value.description}</p>
                  </div>
                </div>
              ))}
            </div>
          </div>
          <div className="card" style={{ order: 1, background: 'linear-gradient(135deg, rgba(37, 99, 235, 0.15), rgba(37, 99, 235, 0.35))' }}>
